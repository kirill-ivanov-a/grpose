...
// THIS IS A FILE that initializes using Sophus, and generatesa a cube of points
// in the world and also some cameras in random locations and then uses OpenGV
// to solve for the translation between two frames
...
// bearingVectors should have good angular distance
opengv::translations_t generate_cube(int cubeDim) {
  double spacing = 1;
  opengv::translations_t pts_in_world;
  double offsetFromOrigin =
      3;  // JULIA: tune this parameter to break the parallax of the
...
int main() {
  ...
  double mindepth = 4;
  double maxdepth = 8;
  // 3d points expressed in world frame
  opengv::translations_t points_world;
  //   for (int i = 0; i < num_inliers; i++) {
  //     points_world.push_back(generateRandomPoint(maxdepth, mindepth));
  //     // std::cout << points_world.back() << std::endl << std::endl;
  //   }

  points_world = generate_cube(
      3);  // 27 points defined in the world, cube of dimension 3x3x3
  ...
  for (int i = 0; i < points_world.size(); i++) {
    ...
    Eigen::Vector3d point_v2_test =
        rotation2.transpose() * (point_v1 - position2);
    ...
    // std::cout << "\nTWO " << cam_rotation.transpose() * (point_v1 -
    // cam_offset)
    //   << std::endl;
    ...
    // std::cout << "\nFOUR " << cam_rotation.transpose() * (point_v2 -
    // cam_offset)
    //   << std::endl;
    ...
    std::cout << "bearing vec 1 \n"
              << (camSE3.rotationMatrix() * bearing_vectors1.back())
              << std::endl;
